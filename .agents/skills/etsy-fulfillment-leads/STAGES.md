...
| 2 · Processing | once, long | `state/*.md`, `state/watchlist.json`, `scripts/taxonomy.py` | no — minutes, needs credentials |
| 3 · Periodic | weekly by default | `radar.md` | **yes — this is the demo** |
...
**Goal:** the recurring run. Sweep every prospect, find who became interesting
*since last time*, say why now.

```bash
python3 scripts/periodic.py              # picks up state/ automatically
```

Check listings and shop data for the Stage 2 keywords, check each tracked metric
against its trigger level, combine into a **ripeness likelihood** — how close
this prospect is to the moment their fulfilment stops working.

### Ripeness is not a yes/no

Report a percentage and a band (RIPE / RIPENING / WATCH / COLD). The underlying
question is continuous: a prospect at 58% is not a "no", it is a "call in three
weeks, and here is what to watch in between".

Report **confidence** separately and define it — the share of triggers that
could be evaluated at all. A prospect can be highly ripe at low confidence.
Conflating the two is how a radar starts lying.

Every point of ripeness traces to a named driver. No unexplained scores.

### Always render

- Prospects that fired, with drivers and evidence.
- Where one qualifies on numbers alone, an explicit statement that there are no
  complaint quotes — and an opener leading with the numbers.
- Prospects that did **not** fire, with reasons. A radar that always finds
  something is not a radar.
...
1. Which did you contact, and which replied?
...
evidence base. Triggers producing replies gain weight; triggers that keep
missing lose it. Record each adjustment in `state/triggers.md` with date and
reason, so the weighting has a history rather than just a value.
